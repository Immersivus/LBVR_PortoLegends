...
Shader "Amazing Assets/Advanced Dissolve/Shader Graph/portal_shader_sphere"
{
Properties
{
...
[Toggle]_SphereMode("Sphere Mode (top = center)", Float) = 0
...
// Sphere portal UV: maps object-space normals to a flat disc UV
// Top of sphere (0,1,0) becomes center (0.5,0.5), equator becomes the rim.
float2 GetPortalUV(float3 objectSpaceNormal, float2 flatUV)
{
    if (_SphereMode > 0.5)
    {
        // Project the upper hemisphere onto a disc using the XZ components of the normal.
        // normalOS is assumed to be unit length; Y=1 is the top pole (center of effect).
        float2 discUV = objectSpaceNormal.xz * 0.5 + 0.5;
        return discUV;
    }
    return flatUV;
}
...
Unity_TilingAndOffset_float(GetPortalUV(IN.ObjectSpaceNormal, IN.uv0.xy), _Property_0547756723fb748b9133a0660e294e71_Out_0_Vector2, (_Multiply_3048a6f0d595da83a6119d7a96f13c95_Out_2_Vector4.xy), _TilingAndOffset_6937019617fac98d9d9b2f666ab3e57d_Out_3_Vector2);
...
//UniversalForward
AdvancedDissolveShaderGraph(GetPortalUV(IN.ObjectSpaceNormal, IN.uv0.xy), IN.ObjectSpacePosition, IN.WorldSpacePosition, IN.AbsoluteWorldSpacePosition, IN.ObjectSpaceNormal, IN.WorldSpaceNormal, float(0), 1, surface.BaseColor, surface.Emission, surface.Alpha, surface.AlphaClipThreshold);
...
//ShadowCaster
AdvancedDissolveShaderGraph(GetPortalUV(IN.ObjectSpaceNormal, IN.uv0.xy), IN.ObjectSpacePosition, IN.WorldSpacePosition, IN.AbsoluteWorldSpacePosition, IN.ObjectSpaceNormal, IN.WorldSpaceNormal, float(0), 1, surface.Alpha, surface.AlphaClipThreshold);
...
//ScenePickingPass
AdvancedDissolveShaderGraph(GetPortalUV(IN.ObjectSpaceNormal, IN.uv0.xy), IN.ObjectSpacePosition, IN.WorldSpacePosition, IN.AbsoluteWorldSpacePosition, IN.ObjectSpaceNormal, IN.WorldSpaceNormal, float(0), 1, surface.BaseColor, surface.Alpha, surface.AlphaClipThreshold);
